...
You can also implement your own custom error formatter. [Learn how »](../developing-extensions/error-formatters.md)

You can change the default error format in the configuration. [Learn how »](../config-reference.md#errorformat)
...
Here's how the parameter can be set in the [configuration file](../config-reference.md):
...
The `editorUrl` parameter also applies to [PHPStan Pro](https://phpstan.orghttps://phpstan.org/blog/introducing-phpstan-pro){.phpstan-pro-label} which lets you browse reported errors (including ignored errors) in a beautiful web UI, and open files in your editor with a click.
...
Since every team member working on the same project will likely have different absolute path to the project, machine-specific `editorUrl` should be used in `phpstan.neon` [paired with `phpstan.neon.dist` where all the common project settings live](../config-reference.md#multiple-files).
