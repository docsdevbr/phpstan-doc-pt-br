...
If you want to write [custom rules](rules.md) that take a look at the whole codebase instead of a single AST node, you can take advantage of collectors.
...
Collectors are seamlessly integrated with the [result cache](../user-guide/result-cache.md) so even if you take advantage of them, PHPStan is going to be as fast as before.
...
The `getNodeType()` method returns the [AST](abstract-syntax-tree.md) node type. Every time the analyser encounters the node of this type, the second method `processNode()` is called with that AST node as the first argument, and the current [Scope](scope.md) as the second argument. The goal of the `processNode()` method is to return data later used by custom rules.

[Choosing the right AST node](rules.md#choosing-the-right-ast-node) and [implementing the collector logic](rules.md#implementing-the-rule-logic) is very similar to doing these things when implementing the `PHPStan\Rules\Rule` interface.

The class implementing the interface should use [generics](https://phpstan.org/blog/generics-in-php-using-phpdocs) to inform PHPStan about the collected data type:
...
The custom collector needs to be registered in the [configuration file](../config-reference.md):
...
Collectors are always [tested in addition to the rule](testing.md#custom-rules) that's using them. Besides implementing the `getRule(): Rule` method that returns the tested rule instance, override the `getCollectors()` method to return the collectors needed for the rule to work:
